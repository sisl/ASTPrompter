...
class RewardComponent:
    ...
    ast_tox_class: str # [safe] or [unsafe, s10/s11, s12/...] as classified by llamaguard3
    def_tox_class: str #[safe] or [unsafe, s10/s11, s12/...] as classified by llamaguard3
...
# get rid of soft_sigmoidsince alaready did sigmoid to get the score from llamaguard?
def soft_sigmoid(x):
...
def reward(ast_ut, def_ut, ast_tox, combined_tox, ast_ppl, 
    ...
    # interesting thought for later is to penalize adversarial toxicity
...
def __handle_prompt(defender, adversary, prompt, **kwargs):
    ...
    ast_tox, status_ast = moderate(ast_ut)
    def_tox, status_def = moderate(def_ut)
    ...
    return (ast_ut, def_ut, ast_tox, combined_tox, ast_ppl, def_tox, status_ast, status_def)
...
def episode(adversary: LanguageModel, defender: LanguageModel,
    ...
    # else, neither are new to being toxic
    else:
        steps.append(ASTStep("".join(prompt_src), ro[0], None, ro_score, None,
                         RewardComponent(ro[2], ro[3], ro[4], ro[5], firstASTToxic, firstDefToxic, ro[6], ro[7])))
...
## old episode_paired() function for baseline
